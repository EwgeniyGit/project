...
namespace EQServiceSystem.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            {
                //return RedirectToLocal(returnUrl); было в оригинале
                var cur_user = model.UserName;
                var role = System.Web.Security.Roles.GetRolesForUser(cur_user).Single();
                ViewBag.Message = role;
                logger.Info("Вход пользователя: {0}", cur_user); 
                return View("~/Views/Home/MainUserPage.cshtml", new MenuLogic(model.UserName));
            }
